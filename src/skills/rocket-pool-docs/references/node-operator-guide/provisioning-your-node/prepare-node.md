<!-- Source: https://docs.rocketpool.net/node-staking/prepare-node -->
...
If so, then you are ready to register your node on the Rocket Pool network and create a megapool with a validator!
If not, please review the previous sections and return here once you've completed those steps.

Before creating a megapool and new validator, there are a few steps to take to finish preparing your node.
These only need to be done once though; once you've done them, you can skip to the [Creating a Megapool Validator](/node-staking/megapools/create-megapool-validator) section if you want to create multiple validators on your node.
...
If you wish to stake RPL you can do this directly on the node, or (preferably) you can use the Rocket Pool website's **Stake on Behalf** function to stake for your node with RPL in your cold wallet.
...
<div className="p-3">
  <Tabs>
    <Tab label="Running on the Hoodi Test Network">
      f you're running on the Hoodi test network, please see the [Practicing with the Test Network](/en/node-staking/testnet/overview) section to learn how to acquire test ETH.

      For test RPL, we have added a similar faucet function directly to the CLI.
      Please see the [Getting Test RPL on Hoodi](/en/node-staking/testnet/overview#getting-test-rpl-on-hoodi) guide to acquire some.
    </Tab>
...
This is the address that all of your RPL checkpoint rewards, your staked RPL, and your Beacon Chain ETH will be sent to when you claim your checkpoint rewards or exit your validator and withdraw from your megapool.
...
The address used to supply a node with ETH should be the primary withdrawal address for the node. Click [here](../legacy/houston/stake-eth-on-behalf.mdx) to read about **staking ETH on behalf of a node**.
...
If you create multiple megapool validators, they will all refer to the same primary withdrawal address so you only need to perform this setup once.
  ...
  <Tabs>
    <Tab label="Method 1">
      **Use this method if your new primary withdrawal address can be used to sign transactions via MetaMask or WalletConnect.**
      ...
      2. To confirm it, you must send a special transaction **from your new primary withdrawal address** to the megapool contract to verify that you own the primary withdrawal address.
    ...
    <Tab label="Method 2">
      **Use this method only if your primary withdrawal address _cannot_ be used to sign transactions via MetaMask or WalletConnect.**
      ...
      If you make a typo here, there is no way to undo it and **your megapool's rewards will be lost forever**.
...
<div className="p-3">
  <Tabs>
    <Tab label="Hoodi Testnet">If you're trying this out on Hoodi testnet, you can sign on this page: https://testnet.node.rocketpool.net/signalling-address</Tab>
    <Tab label="Mainnet">If you're ready to configure this on mainnet, go here instead: https://node.rocketpool.net/signalling-address</Tab>
  </Tabs>
</div>
...
If you create multiple megapool validators, they will all refer to the same RPL withdrawal address so you only need to perform this setup once.
  ...
  <Tabs>
    <Tab label="Method 1">
      *Use this method if your new RPL withdrawal address can be used to sign transactions via MetaMask or WalletConnect.**
      ...
      2. To confirm it, you must send a special transaction **from your new RPL withdrawal address** to the megapool contract to verify that you own the RPL withdrawal address.
...
Once you're done with those topics, you'll be guided through the process of making a megapool validator and earning staking rewards.
