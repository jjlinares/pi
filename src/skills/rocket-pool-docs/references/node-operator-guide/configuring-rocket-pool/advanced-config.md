<!-- Source: https://docs.rocketpool.net/node-staking/advanced-config -->
...
The node daemon exposes a local HTTP API on port 8280 (localhost only). This API is used internally by the CLI and can also be called directly for custom automation or tooling.

The HTTP API port can be adjusted in the Smart Node and TX Fees category under `API port` 

<img src={tui_api_port} width="100%" height="auto" />

External exposure is not currently supported. The API launches only on localhost while we determine the best approach for secure remote access. 
There is no configuration option to expose the port at this time.

You can call the API using standard HTTP requests. 
