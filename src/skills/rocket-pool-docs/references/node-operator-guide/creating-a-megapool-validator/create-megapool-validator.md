<!-- Source: https://docs.rocketpool.net/node-staking/megapools/create-megapool-validator -->
...
Your megapool is automatically deployed at the time of your first validator deposit. Afterwards you'll be able to use the same megapool to manage as many validators as you wish! You won't need
...
The Rocket Pool deposit queue features an express queue system to help existing node operators migrate their minipool validator ETH
...
The deposit queue features an express queue system to help existing node operators migrate their minipool validator ETH
...
If this is your node's first megapool validator, your node's megapool will also be automatically deployed at the same time. Please remember that your node's Megapool can
manage one or many validators, so megapool deployment only happens once per node!
...
Failed transactions (due to adjusted gas settings, insufficient ETH for gas, or the node being offline for 28 days after the initial deposit) could make your megapool validator transition to the `dissolved` state, which you want to avoid.

If a prelaunch validator fails to stake within 365 days, the validator will be dissolved. The 1 ETH (of a 4 ETH bond) sent to the Beacon Chain during the prelaunch process is **not recoverable**.
The node operator receives the remaining 3 ETH from their bond (minus a 0.05 ETH dissolve penalty applied as debt) as a refund in their megapool, for a net refund of 2.95 ETH. 
This can be claimed using `rocketpool megapool claim` or `rocketpool claims claim-all`.
...
for the rest of our demonstration here. Type `1` then hit `enter` to begin creating 1 validator.
...
This shows how many validators are waiting ahead of you to be matched with ETH. The express queue is mainly for already existing node operators as new nodes won't have any express
...
After your `Prelaunch` validator is processed by the Beacon Chain and the initial 1 ETH deposit is credited. Your node will automatically perform a `stake` transaction to complete the full 32 ETH
Beacon Chain deposit. The `stake` transaction will progress your `Prelaunch` validator into a `Staking` validator. At this point your `Staking` validator has: - 32 ETH deposited into the Beacon Chain
- Assigned a validator index number - Pending activation on the Beacon Chain
...
see `Beacon status:                active_ongoing` in the `rocketpool megapool validators` menu to confirm that it's been activated and ready to make attestations.
...
you can do so using the `rocketpool megapool deposit` command. Given the current block gas limit, the maximum amount of validators you can create in one transaction is 35.
